...
| F428 | active | 补齐构建记录的 revision、Commit、Manifest、结果、耗时和逐次日志抽屉 UI。                                              | Build step/read model/log drawer。                                      | 代码/单测/全量/真实 MySQL/认证 HTTP 已闭环；两次独立 Browser 连接均返回 unavailable，AC-BUILD-019～025 保持未勾选，等待当前树 1484x1324 交互/截图/console/network 复验。                                                                                                                                                                                                                                                                                                       |
...
| F435 | done        | 把 Production 确认改成始终弹出的快照确认 Dialog，展示环境、版本、Build、Manifest、配置和策略。                     | Production confirmation Web。                | AC-PROD-001～008；按钮只在生产步骤出现。实现完成、单测/类型/构建/i18n/格式化全部通过（focused 9 测试、全量 57 文件 222 测试、3107 消息 parity）；AC-PROD-001～003 已勾选，AC-PROD-004～008 因本会话无 Browser 工具（同 F428 blocker）保持未勾选，见 f435-evidence-manifest.json。             |
