...
| F418 | done    | 增加项目首页弱摘要读模型：项目形态、环境就绪、资源、入口、Staging/Production 当前版本。                           | 独立 ACL summary read model + SSR/SWR home；默认首页不加载低频详情图，不改发布单详情步骤。                                                                                 | AC-HOME-001～005；canonical identity、F415 intake、环境当前版本、五类资源与 Site 入口均由精确同 team/project/environment 关系计算；真实 MySQL/API/Browser 通过。                                                                   |
