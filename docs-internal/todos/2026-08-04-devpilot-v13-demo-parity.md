...
| F419 | done   | 实现发布单列表搜索、状态筛选、最近执行排序、构建/部署计数和最后执行步骤。                                         | ReleaseOrder list/read model/query/Web list。                                                                                                                              | AC-HOME-006～013；服务端搜索/筛选/排序、稳定分页边界和两次固定查询；真实 MySQL、认证 API/Browser、Console 0、team 隔离及 1484×1324 截图通过，无前端全量拼接。                                                                      |
