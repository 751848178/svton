...
| F107.2 | in_progress | 扩展 Site copy DTO/service：显式 opt-in 时在 dry-run plan 后可排队 live sync，且默认不启用。 | `project-environment` DTO/service。 | 预计新增 `createQueuedLiveSync`、per-site approval/confirmation 和 maxAttempts 透传字段 |
| F107.3 | pending | 补充 ProjectEnvironmentService 回归测试覆盖默认 dry-run 不变、未批准 approval 阻断、已批准 approval 排队。 | `project-environment.service.spec.ts`。 | targeted Jest |
| F107.4 | pending | 更新路线/进度文档，记录 queued live sync 仍不等于自动 worker 完成。 | roadmap/requirements/TODO。 | 保持 live worker/agent 后续缺口可见 |
| F107.5 | pending | 隔离运行 targeted Jest、API type-check/build、touched diff check 和冲突标记扫描。 | API checks。 | 使用 corepack pnpm 8.12.0 |
