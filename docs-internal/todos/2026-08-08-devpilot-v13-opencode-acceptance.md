...
| Version history | repeat Staging without BuildRun growth, upgrades and Staging/Production recoveries | verified with limitation | F555 preview/confirm identity continuity and F537 repo-owned launcher are fixed; current real-MySQL tests pass; isolated current-image runtime execution still needs context; F473/F556-F560 remain |
...
| F533 | P1 leaf/core fixed/F551 remains | evidence/browser; F529, AC-E2E-023 | artifact evidence pins leaf and parent snapshots; F548 now replaces reusable-path preparation with a fresh exclusive run, while the honest transient-swap capability boundary remains F551 | artifact readback/marker parser and fresh run lifecycle only / no browser runtime mutation | `c6d718ff`, `8018a0fa`, `f23ed3b1`; post-review F551 and browser rerun pending |
| F534 | P2 rejected/needs new immutable commit | evidence/browser; F524 | candidate self-tests harden PNG/DOM/text content, but production validator is 237 lines and accepts a valid-IHDR/no-IEND PNG after its 256-chunk bound | artifact content helper/self-test only / no screenshot visual judgment | source `3beb853a` not integrated; no-IEND 3348-byte fixture accepted; split below 200 lines, require complete PNG termination, then re-review |
...
| F540 | P1 core fixed/F551 remains | evidence/provenance; F533, AC-E2E-023 | F548 removes reusable-path pre-pin mutation, hardlink truncation and partial old-batch mutation through a fresh pinned run plus exclusive leaves; same-UID transient swap claims remain explicitly deferred to F551 | browser output directory identity + fresh run lifecycle only / no browser runtime mutation | `8018a0fa`, `f23ed3b1`; independent F548 review pending, F551 remains |
| F541 | P1 fixed with F552-F553 / P3 F554 remains | evidence/identity; F536, AC-E2E-016..023 | base-state binds expected rows to an exact context projection and disjoint Staging/Production identity roots; only insertion-order-sensitive structural equality remains | negative history base-state trusted projection + focused fixtures only | `f0f9b5ee`, `f33418e5`, `9b50ab17`; F542 may resume, F554 remains fail-closed quality debt |
| F542 | P1 core fixed/follow-ups F555-F560 | evidence/identity; F536, AC-E2E-016..023 | all 15 mapped steps bind to trusted roots, but post-review found unbound preview hashes, self-derived route expectations, ignored Production prefixes, unenforced result inventories and missing EnvironmentVersion edges | negative history trusted per-step expectation graph + producer-shaped fixture only | `3c5c57fb`; independent adversarial review produced F555-F560, so the identity graph is not yet accepted |
...
| F551 | P2 confirmed/queued | evidence/provenance; F540 | a directory swap restored before the next assertion is invisible to before/after inode checks while the path-based producer may have written elsewhere | browser producer output capability/run-directory lifecycle only | swap-restore repro / unique unpublished run directory or descriptor-relative producer writes; do not claim transient-swap proof otherwise |
...
| F554 | P3 confirmed/queued | evidence/quality; F541 | route snapshot equality uses insertion-order-sensitive `JSON.stringify`, rejecting semantically identical object key order | shared JSON structural equality helper + focused tests only | pure Node repro / compare object key sets recursively while preserving array order and primitive exactness |
...
| F559 | P2 reviewed/snapshot verified; exact integration conflict | evidence/identity; F542, AC-E2E-017..021 | immutable source binds four upgrade/recovery row foreign keys and adversarial cases pass, but exact cherry-pick conflicts in the legacy producer | version-history producer serialization + four action validators/fixtures only | source `b9d03f56` not integrated: `scripts/parity-version-history-e2e.mjs` conflict, abort verified; require a new clean commit rebased on accepted ancestry, do not hand-merge |
| F560 | P3 confirmed/queued | evidence/quality; F542 | one global distinctness check mixes unrelated Prisma model IDs and can reject schema-valid equal strings across tables | generated-role distinctness grouping/self-test only | cross-table equal IDs false-rejected / require uniqueness separately per entity type |
