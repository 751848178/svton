...
| F513 | P1 confirmed/queued | evidence script; F474, AC-E2E-024..035 | negative context accepts legacy F456 `fixedIds`, while current F456 writes `context`; stale artifact passes and fresh artifact fails | negative context/check helper only / no E2E mutation | F474 post-review + current `/tmp/f456` proof / require current schema, exact AC/source-step checks, capturedAt and provenance |
...
| F523 | P1 confirmed/queued | evidence/browser; F507, AC-E2E-023 | missing `console`/`httpResponses`/`failedRequests` fields default to empty arrays and appear successful | browser evidence schema/checks only / no hash binding overlap with F511 | F507 post-review / require explicit arrays and schema/version before zero-error assertions |
| F524 | P1 confirmed/queued | evidence/browser; F507, AC-E2E-023 | empty screenshot/DOM/text artifacts still have valid SHA-256 and satisfy the hash-only gate | browser artifact manifest/checks only / no visual parity redesign | F507 post-review / require byte count and state-specific content markers alongside hashes |
| F525 | P2 confirmed/queued | tests; F507 | self-tests omit Runtime exception, missing fields, empty artifacts, Document error and exact URL/host mapping | CDP/browser evidence self-tests only / no runtime behavior | F507 post-review / add adversarial coverage with the corresponding fixes |
| F526 | P1 confirmed/queued | evidence script; F509, F455 context | all nine ACs can coherently reference one fake passing step because canonical per-AC sourceSteps are not enforced | history context/check helper only / no positive runner behavior | pure Node repro / bind exact canonical AC-to-step mapping |
| F527 | P1 confirmed/queued | evidence script; F509 | duplicate source step/check names and matching duplicate checkNames are accepted | history context/check helper only / no positive runner behavior | pure Node repro / require unique sourceSteps, unique step check names and unique expanded checkNames |
| F528 | P2 confirmed/queued | tests; F509 | self-tests miss coherent mapping substitution and duplicate-name attacks | history context self-test only / no runtime behavior | post-review / add canonical substitution and duplicate fixtures with F526/F527 |
