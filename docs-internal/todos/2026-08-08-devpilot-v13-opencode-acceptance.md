...
| F533 | P1 fixed with F540 | evidence/browser; F529, AC-E2E-023 | artifact evidence pins each leaf and its parent browser output directory, derives hashes plus markers from one byte snapshot and rejects symlink/replacement/read drift | artifact readback/marker parser only / no browser runtime mutation | `c6d718ff`, `8018a0fa`; browser rerun still pending isolated current-HEAD stack |
...
| F540 | P1 fixed | evidence/provenance; F533, AC-E2E-023 | browser output now performs pre-mutation no-symlink containment checks, retains a directory FD bound to `dev/ino/nlink/mode`, and revalidates it around producer, every leaf snapshot and CDP evidence | browser output directory identity + artifact readback only / no browser runtime mutation | `8018a0fa`, 21-case pure filesystem matrix; isolated browser rerun still pending |
