...
| F536 | P1 core fixed/follow-up F542 | evidence script; F513, AC-E2E-024..035 | current-history intake replays canonical checks and F541 now binds trusted base anchors, but mapped action identities still require the F542 cross-step graph | negative history contract/shared history-check contract only / no F456 runtime behavior | `f31a1f3e`, `f0f9b5ee`; generic/base substitutions closed, mapped cross-step identity remains F542 |
...
| F541 | P1 core fixed/follow-ups F552-F554 | evidence/identity; F536, AC-E2E-016..023 | base-state binds expected rows to trusted context, but post-review found extra-context overwrite, cross-environment version alias and object-order equality gaps | negative history base-state trusted projection + focused fixtures only | `f0f9b5ee`; F552-F554 registered before F542 resumes |
| F542 | P1 confirmed/queued | evidence/identity; F536, AC-E2E-016..023 | mapped action checks derive both actual and expected identities from the same untrusted result, so coherent cross-step environment/manifest/version substitutions pass | negative history trusted per-step expectation graph + producer-shaped fixture only | pure Node repro / derive staging, Production, manifest and previous-version identities from context and preceding validated outputs |
...
| F552 | P1 confirmed/queued | evidence/provenance; F541, F537 | extra `document.context` keys are accepted and spread after validated provenance fields, allowing context to overwrite `status`, `sourcePath` and related consumer metadata | negative history context allowlist/return projection only | pure Node repro / require exact context key set and construct an allowlisted return projection without overwrite |
| F553 | P1 confirmed/queued | evidence/identity; F541 | the same globally keyed EnvironmentVersion ID can be accepted as both Staging Vst1 and Production Vprod1 because uniqueness is checked per array only | negative history base identity disjointness/self-test only | pure Node repro / require distinct env/current IDs and reject duplicates across combined version arrays |
