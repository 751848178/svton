...
| F541 | P1 fixed | evidence/identity; F536, AC-E2E-016..023 | base-state now binds every expected context field plus B1/M1/D1, Staging/Production versions, environment pointers, commit/digest/route roots and derived Production R1 to trusted context | negative history base-state trusted projection + focused fixtures only | `f0f9b5ee`; F456 producer also serializes Staging `deploymentRunId`, coherent base substitutions rejected |
