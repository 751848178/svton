...
| F543 | P1 fixed/post-review incomplete | evidence/security; F539, AC-E2E-023 | explicit auth/proxy-auth/cookie/set-cookie values are collected as non-overlapping original-text spans and replaced once, preserving true boundaries while covering custom/no scheme, cookie pairs and attributes | CDP header/key-value redaction grammar + focused tests only | `5755b4ce`; cost-closure rerun passed, but the independent adversarial review was stopped before verdict and remains a continuation item |
