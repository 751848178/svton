...
| F543 | P1 core fixed/post-review complete; F561 remains | evidence/security; F539, AC-E2E-023 | explicit auth/proxy-auth/cookie/set-cookie values are collected as non-overlapping original-text spans and replaced once; adversarial review confirmed secret removal, overlap handling and idempotence, but adjacent inline safe fields can be fail-closed over-redacted | CDP header/key-value redaction grammar + focused tests only | `5755b4ce`; review log `parallel-wave-1/codex-f544/f543-adversarial-review.log`; F561 tracks the preservation-boundary defect |
...
| F558 | P2 confirmed/queued | evidence/schema; F542, AC-E2E-016..023 | exact result-key inventory is used only by a self-test and already omits current browser fields, so runtime parsing accepts unknown provenance keys | negative-history runtime result inventory + producer-shape tests only | forged extra result key accepted / enforce exact per-step result keys and synchronize delegated browser shape |
| F559 | P2 confirmed/queued | evidence/identity; F542, AC-E2E-017..021 | four upgrade/recovery receipts omit selected EnvironmentVersion `artifactManifestId` and `deploymentRunId`, so row identity is not bound to the action | version-history producer serialization + four action validators/fixtures only | selected DB fields disappear from evidence / serialize and bind both foreign keys to validated action identities |
...
| F561 | P2 confirmed/queued | evidence/quality; F543 | line-start colon fields and equals-delimited auth/cookie values consume adjacent safe fields such as `Authorization=Custom secret, safe=visible` and `Cookie=sid=secret; safe=visible`, destroying non-secret evidence while still removing the secret | CDP header-value boundary scanner and focused self-test only / no vocabulary or capture expansion | reproduce colon/equals plus comma/semicolon adjacency, custom schemes, Expires, quotes/escapes and overlapping spans / preserve adjacent safe fields, redact the full sensitive value, remain idempotent, and keep F530/F539/F543 regressions green |
