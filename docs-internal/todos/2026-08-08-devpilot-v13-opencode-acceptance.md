...
| F533 | P1 leaf/core fixed/follow-ups F548-F551 | evidence/browser; F529, AC-E2E-023 | artifact evidence pins leaf and parent directory snapshots, but reusable path preparation still has pre-pin mutation and restored-swap gaps | artifact readback/marker parser only / no browser runtime mutation | `c6d718ff`, `8018a0fa`; post-review follow-ups F548-F551, browser rerun pending |
...
| F540 | P1 core fixed/follow-ups F548-F551 | evidence/provenance; F533, AC-E2E-023 | browser output retains and revalidates directory identity around reads, but post-review found pre-pin mutation races, hardlink truncation, partial batch mutation and restored transient swaps | browser output directory identity + artifact readback only / no browser runtime mutation | `8018a0fa`; shift follow-up design to a fresh unique mode-0700 run directory rather than reusable-path repair |
...
| F547 | P1 fixed/review pending | evidence/security; F507, AC-E2E-023 | raw actions now remain in-memory for execution while CDP evidence v2 persists only exact nonempty descriptors without setValue values or raw navigate URLs | CDP driver action serialization/redaction + focused evidence tests only | `e6c93fb9`; 8 self-test groups and static rawActions/schema guards passed, independent post-commit review pending |
| F548 | P1 confirmed/queued | evidence/provenance; F540 | detached preflight is followed by path-based profile removal and leaf creation before the retained directory pin, so an ancestor swap can delete or truncate outside the intended output | browser run-directory allocation/preparation only | ordinary-file repro / allocate a fresh unique mode-0700 run-owned output directory and pin it before producer use |
| F549 | P1 confirmed/queued | evidence/safety; F540 | precreation opens existing leaf paths with `O_TRUNC` before identity/nlink validation; a hardlink leaf truncates an external victim | safe leaf creation/preparation helper only | hardlink repro / avoid reusable leaves or validate lstat+fstat+nlink before handle truncation |
| F550 | P2 confirmed/queued | evidence/safety; F540 | batch preparation truncates earlier leaves before a later unsafe leaf rejects, leaving partial mutation on a failed run | browser leaf preparation transaction/journal only | two-leaf repro / fresh empty run directory or all-target prevalidation before mutation |
