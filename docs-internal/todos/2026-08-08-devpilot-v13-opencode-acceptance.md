...
| F556 | P1 confirmed/queued | evidence/identity; F542, AC-E2E-020..021 | route proof derives domains, proxy, TLS, final URL, routeHash and operationId from its own claimed expected object instead of the trusted Production route snapshot | negative-history Production route expectation reconstruction only | coherent alternate-route proof accepted / rebuild all derived fields from trusted route roots and independently validated site/provider inputs |
| F557 | P1 confirmed/queued | evidence/identity; F542, AC-E2E-022 | Production version-chain validation finds Vprod1 and ignores every preceding row while trusting a producer boolean | Production version-chain consumer/fixtures only | malformed prefix accepted / require the current exact three-row chain or validate every prefix row and link without trusting `chainLinksValid` |
