...
| F536 | P1 core fixed/follow-ups F541-F542 | evidence script; F513, AC-E2E-024..035 | current-history intake replays canonical checks and exactly compares serialized fields, but post-review proved result-owned expected identities can still be coherently substituted | negative history contract/shared history-check contract only / no F456 runtime behavior | `f31a1f3e`; generic claim/field/order drift closed, trusted base/cross-step identity binding remains F541-F542 |
...
| F541 | P1 confirmed/queued | evidence/identity; F536, AC-E2E-016..023 | `base-state-rows` can substitute staging/Production environment and version IDs in both rows and `result.expected` while the accepted top-level context remains unchanged | negative history base-state trusted projection + focused fixtures only | pure Node repro / bind every base expected identity to `document.context` before canonical replay |
