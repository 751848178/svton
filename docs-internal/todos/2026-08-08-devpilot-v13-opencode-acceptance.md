...
| F533 | P1 fixed | evidence/browser; F529, AC-E2E-023 | artifact evidence rejects symlink/non-regular files, pins one no-follow file handle and derives hashes plus markers from the same verified byte snapshot | artifact readback/marker parser only / no browser runtime mutation | `c6d718ff`, path replacement/read drift/FIFO/symlink fixtures; browser rerun still pending isolated current-HEAD stack |
