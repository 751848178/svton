...
| F530 | P1 core fixed/follow-ups F543-F546 | evidence/security; F522, AC-E2E-023 | Runtime exception sanitizer covers common credential fields and full URL fragments, but post-review found custom auth/cookie parsing, non-exception capture branches, separator and over-redaction gaps | CDP capture/redaction helpers and focused tests only / no browser product fix | `7c4d12a1`, `8885b24f`, `0568fd33`; explicit follow-ups registered, not claimed as generic DLP |
...
| F539 | P1 core fixed/follow-ups F543-F546 | evidence/security; F530, AC-E2E-023 | centralized vocabulary covers common Cookie/session/API-key fields and strict whole-fragment clearing, but post-review found additional capture and parsing bypasses | CDP redaction vocabulary/self-test only / no generic DLP expansion | `8885b24f`, `0568fd33`; 42-case post-review produced F543-F546 |
...
| F543 | P1 confirmed/queued | evidence/security; F539, AC-E2E-023 | custom `Authorization`/`Proxy-Authorization` schemes and embedded multi-pair Cookie text can preserve credential tails | CDP header/key-value redaction grammar + focused tests only | pure Node repro / redact entire auth header value independent of scheme and entire embedded Cookie payload |
...
| F548 | P1 fixed/review pending | evidence/provenance; F540 | each browser pass now creates and immediately pins a fresh unpredictable mode-0700 direct child before any producer mutation; no reusable profile/output cleanup remains | browser run-directory allocation/preparation only | `f23ed3b1`; 100-run uniqueness, zero-old-mutation and focused safety matrices passed; independent review pending |
| F549 | P1 fixed by F548/review pending | evidence/safety; F540 | artifact and CDP evidence leaves are created only through O_EXCL/O_NOFOLLOW mode-0600 writes inside the fresh run, so stale hardlink/symlink/FIFO names fail without truncating a target | safe leaf creation/preparation helper only | `f23ed3b1`; hardlink/symlink/FIFO/EEXIST matrix passed; independent review pending |
| F550 | P2 fixed by F548/review pending | evidence/safety; F540 | there is no old-leaf preparation batch; partial or injected producer failure is confined to the new failed run and cannot mutate prior evidence | browser leaf preparation lifecycle only | `f23ed3b1`; partial/injected failure and zero-old-mutation tests passed; independent review pending |
