...
| F536 | P1 fixed | evidence script; F513, AC-E2E-024..035 | current-history intake now replays every mapped step plus `base-state-rows` through the canonical registry and exactly compares ordered `name/pass/actual/expected` fields | negative history contract/shared history-check contract only / no F456 runtime behavior | `f31a1f3e`, all 15 coherent claim substitutions plus field/order/result drift fixtures rejected |
