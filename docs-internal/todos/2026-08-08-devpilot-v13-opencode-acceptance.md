...
| F474 | P1 fixed/pending post-review | evidence script; F457, AC-E2E-024..035 | 46 negative steps now require named checks; 12 ACs derive from exact mapped steps | negative runner/helpers only / no product behavior | `04c7ff91`; static/adversarial self-tests green, runtime rerun waits for isolated current stack |
...
| F506 | P1 confirmed/queued | evidence script; F473, AC-E2E-018..021 | six history validators accept missing identities/digests via `undefined === undefined` | history evidence helper/self-test only / no runtime behavior | post-review pure-Node repro / require presence and exact independent identities |
| F507 | P1 confirmed/queued | evidence/browser; F473, AC-E2E-023 | CDP parser checks `level=error` instead of `type=error` and ignores Fetch/XHR 4xx/5xx | history browser driver/evidence tests only / no UI source fix | post-review captured event shape / fail on console error and all relevant HTTP failures |
| F508 | P2 confirmed/queued | evidence script; F473 | login is a handwritten passed step outside checkedStep | history runner/helper only / no auth product fix | post-review source trace / route login through verified step without persisting token |
| F509 | P2 confirmed/queued | evidence script; F473, F455 context | producer context accepts any eight/previously any non-empty true ACs instead of exact AC-E2E-007..015 | history context helper only / no positive runner behavior | post-review self-test gap / require exact AC ID set and evidence hash |
