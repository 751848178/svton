...
- F55 completed contextual operation entry links from the environment workbench
  ...
  context.
- Current source-backed structure result: project-environment and resource-control
  backend non-spec files are now under the 200-line ceiling after F285/F274.
  New work should move to the next docs-backed capability or module gap rather
  than continuing a P1 line-ceiling split.
...
## F274. Resource Control Controller Split (module ceiling reached)

Purpose: finish the resource-control module by bringing the 413-line
`ResourceControlController` under the 200-line ceiling. Source inspection
confirmed the over-ceiling size came from 17 route handlers + ~89 lines of
private access-policy helpers (`assertCanWriteResource`/`assertCanReadResource`/
`filterReadableResourceRecords`/`getReadableResourceScope`/`getMetadataAccessScope`)
plus two inline sync-write asserts. This slice extracts the access-policy
helpers into a focused service, then splits the routes into read and write
controllers (NestJS supports multiple controllers on the same base path).

| Task   | Status | Description                                                                | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------ | ------ | -------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F274.1 | done   | Map the controller over-ceiling cause.                                     | Manual graph confirmed the size came from route handlers + private access-policy helpers + two inline sync asserts. The helpers depend only on the injected `ControlAccessPolicyService`, so they extract cleanly. Routes split into read (11 GET) and write (1 PUT + 5 POST).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| F274.2 | done   | Extract access-policy service + split controller into read/write.          | New `ResourceControlAccessPolicyService` (153 lines: `assertCanWriteResource`/`assertCanReadResource`/`filterReadableResourceRecords`/`getReadableResourceScope`/`getMetadataAccessScope` + new `assertCanSyncDocker`/`assertCanSyncCloud` to cover the two inline sync writes + `ResourceControlAuthRequest`/`ReadableResourceRecord` types). Controller file rewritten as two classes: `ResourceControlReadController` (11 GET routes) + `ResourceControlWriteController` (6 mutating routes), both delegating access checks to the new service. Module registers both controllers + the new provider; controller spec rewired to construct both controllers with a real `ResourceControlAccessPolicyService` wrapping the mocked `ControlAccessPolicyService`. Controller file dropped from 413 to 168 lines. |
| F274.3 | done   | Run focused API verification and hygiene checks, then sync final evidence. | Focused resource-control Jest passed (63 tests, 12 suites, incl. controller spec covering both controllers): `/tmp/codex-tool-runs/svton/f274-jest-20260707.log`; API type-check passed (0 errors): `/tmp/codex-tool-runs/svton/f274-tc1-20260707.log`; controller 168 + access-policy 153 (both ≤200); `git diff --check` clean; conflict-marker scan clean; single-quote API convention preserved. **resource-control module ceiling reached: every non-spec file in the directory is now ≤200 lines.**                                                                                                                                                                                                                                                                                                        |
...
## F285. Project Environment Controller Route/Access Split

Purpose: finish the project-environment module line-ceiling pass by splitting
the remaining 385-line controller route/access layer while preserving the
`project-environments` base path, route handlers, access-policy payloads, DTO
contracts, service calls, Prisma schema, and Web UI behavior.

| Task   | Status | Description                                                      | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------ | ------ | ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F285.1 | done   | Map the controller route/access boundary.                        | Routing: focused slice + noisy-tools verification; no multi-agent needed because this is confined to P1 ProjectEnvironment controller/module/spec/docs. CodeGraph is installed but uninitialized, so this slice uses a manual graph. Manual graph confirmed the controller is a thin route layer with 3 read routes, 11 write routes, and repeated access-policy payload shaping.                                                                                                                                                                                                                                                                                                                                                                     |
| F285.2 | done   | Split read/write controllers and focused access-policy services. | `project-environment.controller.ts` is now a compatibility barrel; read routes live in `project-environment-read.controller.ts` (73), write routes in `project-environment-write.controller.ts` (188), read access filtering in `project-environment-read-access-policy.service.ts` (58), write gates in `project-environment-write-access-policy.service.ts` (167), cross-environment copy/apply gates in `project-environment-copy-access-policy.service.ts` (116), and access request/scope types in `project-environment-access-policy.types.ts` (20). Module registers both controllers and three access-policy services.                                                                                                                        |
| F285.3 | done   | Run focused verification and sync P1/TODO/roadmap/refactor docs. | Focused ProjectEnvironment controller Jest passed: `/tmp/codex-tool-runs/svton/f285-project-environment-controller-jest-20260710.log`; API type-check passed: `/tmp/codex-tool-runs/svton/f285-api-type-check-20260710.log`; API build passed: `/tmp/codex-tool-runs/svton/f285-api-build-20260710.log`; Prettier, line-count, diff check, conflict-marker scan, and trailing-whitespace scan passed: `/tmp/codex-tool-runs/svton/f285-prettier-check-20260710.log`, `/tmp/codex-tool-runs/svton/f285-project-environment-line-count-20260710.log`, `/tmp/codex-tool-runs/svton/f285-diff-check-20260710.log`, `/tmp/codex-tool-runs/svton/f285-conflict-scan-20260710.log`, `/tmp/codex-tool-runs/svton/f285-trailing-whitespace-scan-20260710.log`. |
...
### Project-environment module (current, post-F285)
...
Organization map (file → responsibility, non-spec files all ≤200 lines):

- `project-environment.controller.ts` (2) — compatibility barrel for the split controllers.
- `project-environment-read.controller.ts` (73) — read routes + sync-suggestion list route.
- `project-environment-write.controller.ts` (188) — create/update/archive, sync apply, copy, bulk-bind, server bind/unbind, sync-from-project routes.
- `project-environment-read-access-policy.service.ts` (58) — read assertion and readable-environment filtering.
- `project-environment-write-access-policy.service.ts` (167) — project/environment write gates and server binding gates.
- `project-environment-copy-access-policy.service.ts` (116) — cross-environment copy/apply read-source + write-target gates.
- `project-environment-access-policy.types.ts` (20) — controller/access request and scope types.
...
## Gaps Identified From The Module Maps (post-F285)
...
- **Controller size — RESOLVED in F285:** the controller route/access layer is
  split into read/write controllers plus focused access-policy services; every
  project-environment non-spec file is now ≤200 lines.
- **No behavioral test gaps added:** all extracted boundaries retain their
  pre-existing behavioral coverage (controller authorization spec green through F285);
...
- **project-environment module: ceiling reached (F285).** Every non-spec file in
  the directory is ≤200 lines; future P1 work should be capability-driven.
- **resource-control module: ceiling reached (F274).** Every non-spec file in
  the directory is ≤200 lines (host facade 75, scheduler 172, read+write
  controllers 168, access-policy service 153, all focused services/utils ≤200).
  The host `ResourceControlService` is a thin facade; its behavior boundaries
  (binding/query/connection/action/inventory/metrics) live in focused services.
- **Move to further modules (逐模块):** site / monitoring / log-center /
  ops-governance god services — each needs its own split pass + module map set
  (business logic / organization / function / data-flow / page structure).
