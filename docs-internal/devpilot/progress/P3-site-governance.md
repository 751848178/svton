...
## F283. Site Service Facade Convergence

Purpose: finish the current backend `site.service.ts` split by moving the
remaining CRUD/query/binding and operation-plan orchestration methods into
focused services while preserving the public `SiteService` facade, route/DTO
contracts, approval/queue/execution behavior, Prisma schema, and Web UI behavior.

Assumption: F283 is a structure slice only. It does not change Site action
labels, dry-run defaults, queue/maxAttempts propagation, rollback eligibility,
TLS follow-up callback behavior, audit semantics, or controller routes.

| Task   | Status | Description                                                       | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------ | ------ | ----------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F283.1 | done   | Map remaining SiteService CRUD/query and operation-plan seams.    | Routing: focused slice + noisy-tools verification; no multi-agent needed because the change is confined to P3 `site` backend files and docs. CodeGraph is installed but uninitialized, so this slice uses a manual graph from source symbols and focused specs. Manual graph confirmed the remaining host seams are CRUD/query/binding, takeover coordination, operation-plan dispatch, and TLS/rollback dispatch.                                                                                                                                                                                                                                                                                          |
| F283.2 | done   | Extract CRUD/query and operation-plan services behind the facade. | `SiteCrudService` now owns list/create/get/listSyncRuns/update/delete and takeover update persistence; `SiteBindingService` owns project/environment/server/proxy binding checks; `SiteOperationPlanService` owns sync/diagnostics/OpenResty/smoke execution-plan dispatch; `SiteTlsRollbackOperationService` owns TLS probe/renew and rollback dispatch; `SiteService` is a 167-line public facade that preserves the existing constructor signature and callback wiring.                                                                                                                                                                                                                                  |
| F283.3 | done   | Run focused verification and sync P3/TODO/roadmap/refactor docs.  | Focused site Jest passed: `/tmp/codex-tool-runs/svton/f283-site-jest-20260710.log`; API type-check passed: `/tmp/codex-tool-runs/svton/f283-api-type-check-20260710.log`; API build passed: `/tmp/codex-tool-runs/svton/f283-api-build-20260710.log`; Prettier, line-count, diff check, conflict-marker scan, and trailing-whitespace scan passed: `/tmp/codex-tool-runs/svton/f283-final-prettier-check-20260710.log`, `/tmp/codex-tool-runs/svton/f283-site-line-count-20260710.log`, `/tmp/codex-tool-runs/svton/f283-final-diff-check-20260710.log`, `/tmp/codex-tool-runs/svton/f283-final-conflict-scan-20260710.log`, `/tmp/codex-tool-runs/svton/f283-final-trailing-whitespace-scan-20260710.log`. |

## F284. Site DTO Family Split

Purpose: finish the current backend Site line-ceiling pass by splitting
`dto/site.dto.ts` by route family while preserving public DTO class names,
validator decorators, route payloads, controller/service import paths, Prisma
schema, and Web UI behavior.

Assumption: F284 is a structure slice only. It does not change
`SITE_RUNTIME_TYPES`, `SiteRuntimeType`, DTO field names, validation decorators,
or the public `./dto/site.dto` export surface.

| Task   | Status | Description                                             | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------ | ------ | ------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F284.1 | done   | Map Site DTO exports/imports and split families.        | Routing: focused slice + noisy-tools verification; no multi-agent needed because this is confined to P3 Site DTO files and compatibility re-exports. CodeGraph is installed but uninitialized, so this slice uses a manual graph. Manual graph confirmed 13 DTO classes + `SiteRuntimeType` are consumed through `./dto/site.dto` by Site controller/facade/focused services and plan utils.                                                                                                                                                                                                                                                                                        |
| F284.2 | done   | Split DTO definitions while preserving the public path. | `site-crud.dto.ts` owns list/create/update DTOs; `site-operation.dto.ts` owns sync/takeover/diagnostics/OpenResty/smoke DTOs; `site-tls-rollback.dto.ts` owns TLS probe/renew, sync-run query, and rollback DTOs; `site-runtime.constants.ts` + `site-runtime.types.ts` own runtime values/types; `site.dto.ts` remains a compatibility barrel. Largest new DTO file is 148 lines.                                                                                                                                                                                                                                                                                                  |
| F284.3 | done   | Run focused verification and sync P3/TODO/roadmap docs. | Focused site Jest passed: `/tmp/codex-tool-runs/svton/f284-site-jest-20260710.log`; API type-check passed: `/tmp/codex-tool-runs/svton/f284-api-type-check-20260710.log`; API build passed: `/tmp/codex-tool-runs/svton/f284-api-build-20260710.log`; Prettier, line-count, diff check, conflict-marker scan, and trailing-whitespace scan passed: `/tmp/codex-tool-runs/svton/f284-prettier-check-20260710.log`, `/tmp/codex-tool-runs/svton/f284-site-line-count-20260710.log`, `/tmp/codex-tool-runs/svton/f284-diff-check-20260710.log`, `/tmp/codex-tool-runs/svton/f284-conflict-scan-20260710.log`, `/tmp/codex-tool-runs/svton/f284-trailing-whitespace-scan-20260710.log`. |

## Site Module Backend Maps (current, post-F284)
...
- `site.controller.ts` (156) — HTTP routes split into read/write controllers.
- `site.service.ts` (167) — public Site facade that preserves controller and
  scheduler-facing methods while delegating focused work (F283).
- `site-crud.service.ts` (176) — Site list/create/get/listSyncRuns/update/delete
  plus takeover update persistence (F283).
- `site-binding.service.ts` (59) — project/environment/server/proxy binding
  existence checks (F283).
- `site-operation-plan.service.ts` (190) — sync/diagnostics/OpenResty/smoke
  operation-plan dispatch into `SiteSyncExecutionService` (F283).
- `site-tls-rollback-operation.service.ts` (126) — TLS probe/renew and rollback
  dispatch into `SiteSyncExecutionService` (F283).
...
- `site-tls-probe.ts` (200) + `site-tls-renew.ts` (141)
  — TLS probe/renew metadata + command builders.
- `site-tls-probe-scheduler.service.ts` (199) + `site-tls-renew-scheduler.service.ts`
  (194) — scheduled TLS probe/renew ticks.
- `dto/site.dto.ts` (18) — compatibility barrel preserving public DTO import path.
- `dto/site-crud.dto.ts` (115) — list/create/update Site DTOs (F284).
- `dto/site-operation.dto.ts` (148) — sync/takeover/diagnostics/OpenResty/smoke
  operation DTOs (F284).
- `dto/site-tls-rollback.dto.ts` (80) — TLS probe/renew, sync-run query, and
  rollback DTOs (F284).
- `dto/site-runtime.constants.ts` (1) + `dto/site-runtime.types.ts` (3)
  — Site runtime values/types (F284).
...
## Gaps Identified From The Site Backend Maps (post-F284)

- Site backend non-spec runtime and DTO files are at or below the 200-line
  ceiling after F284; existing specs remain intentionally larger.
- No current source-backed P3 Site backend structure gap remains in the mapped
  files. Future Site work should be capability-driven rather than another
  line-ceiling split unless new source evidence changes this map.
