...
- [ ] **AC-DIR-001** Sidebar 只有一个项目模块入口，不固定任何具体项目。
- [ ] **AC-DIR-002** `/projects` 默认显示项目总数、线上运行数和待配置数。
- [ ] **AC-DIR-003** 支持按项目名称、仓库和域名搜索。
- [ ] **AC-DIR-004** 状态是主要筛选项；筛选名称与用户可见项目状态一致。
- [ ] **AC-DIR-005** 项目默认按最近活动排序，不要求用户手工调整顺序。
- [ ] **AC-DIR-006** 每行显示名称、项目形态、环境基线、Production 版本/域名和最近活动。
- [ ] **AC-DIR-007** 每个项目只有一个明确的“进入项目”动作。
- [ ] **AC-DIR-008** 目录空态同时提供“生成新项目”和“接入已有项目”入口。
