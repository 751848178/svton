...
## Third Architecture Review Closure (2026-08-11)

- Gate evaluations retain the complete refreshable attempt hash, while additive `approvalSubjectHash` binds reusable manual approvals to the stable checkpoint subject. Final reservation rechecks the current attempt, subject, SourcePolicy threshold, freshness and distinct reviewers; legacy waivers remain separate.
- P09 reads only typed candidate hash, observation time, probe hash and immutable probe payload. Generic route-result JSON cannot satisfy the post-route gate. Acceptance readiness also verifies a canonical server-owned scanner/base/rules/database supply proof and fails closed with `build_worker_supply_digest_unverified` when it is missing or altered.
- The filesystem acceptance worker is split into a trusted supervisor and fixed untrusted broker entry. The supervisor alone reads the HMAC secret, scans before repository commands, validates broker artifacts and signs the final result; the broker runs as uid/gid 3000 with a minimal environment and inherited standard descriptors only. Linux uid isolation tests are defined but remain unexecuted on the non-root macOS host because this round prohibited Docker.
