...
“管理项目”承载低频设置：仓库与识别、环境、资源、Webhook、发布策略和项目资料。项目完成接入后，仓库提供商和规范地址只读；凭据重连不会改变仓库身份，默认分支只能通过带原因、权限、预期修订和审计的高风险修订操作更新。每个 BuildRun 固定身份修订、分支和精确 Commit。READY 历史项目若缺少可验证的身份修订，会关闭仓库修改和构建并显示迁移提示，不会猜测补齐。环境配置每次保存创建不可变修订；Secret 只保存引用，不把明文写入配置快照、门禁证据或日志。
