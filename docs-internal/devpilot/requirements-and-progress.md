...
- 监控、日志、备份、审批、审计和访问策略已经具备第一版闭环，资源级指标时间序列曲线已补入资源详情，日志中心已支持入库日志 SSE 流式 tail、cursor resume、断线自动重连、有界会话治理、活跃会话控制、单流/用户/团队基础限流和默认关闭的 Server executor 定时 follow，且手动关闭日志流会话已写入审计，Agent Follow UI 的 metadata 持久化契约已补齐，监控页也已具备资源指标大盘、服务 SLO 大盘、服务 SLO `applicationServiceId` 精确过滤、应用服务页单服务 SLO 摘要、监控页单服务 SLO 详情深链、站点 Smoke 失败告警、部署 Smoke 失败告警、服务 SLO 违约告警、短/长窗口 burn-rate 策略、错误预算阈值策略、错误预算耗尽预测、SLO 模板和事件去重抑制第一版，通用 Webhook、飞书、钉钉、企业微信机器人通知、邮件通知、失败/planned 投递手动重试、默认关闭的失败通知自动重试以及默认关闭的严重告警升级第一版也已补齐；Monitoring Web 当前源码文件均低于 200 行，API DTO 输入结构已拆成 alert、dashboard、silence、notification focused DTO 文件并保留 `monitoring.dto.ts` barrel contract，Monitoring scheduler 已拆出 config service、summary types 和 summary value utils，主 scheduler service 保留编排入口且低于 200 行，Monitoring HTTP 入口已拆成 alert rules/events、dashboard、silence、notification focused controllers 并通过 shared access provider 复用读写访问策略，资源指标大盘 read-model 已拆出 focused query service、builder service 和类型文件且保留 MonitoringService 兼容委托，服务 SLO 大盘 read-model 已拆出 focused query service、builder service、status service、类型文件和 derived metric 常量且保留 MonitoringService 兼容委托，notification delivery read/access-scope 查询已拆出 focused read service、shared include 常量和 delivery record 类型且保留 MonitoringService 兼容委托，notification delivery payload/text/email formatting 已拆出 focused builder/types/utils/channel select 边界，notification delivery dispatch/config/write execution 已拆出 focused dispatch、Webhook/Email adapter、sender、writer 和 config service，manual/scheduled notification retry orchestration 与 retry audit 已拆出 focused retry service、audit service、types 和 utils，stale alert escalation scan/dedupe/audit 已拆出 focused escalation service、audit service、types 和 utils，notification-channel list/access-scope/create/update/settings 已拆出 focused channel service/settings service/filter/json utils 且保留 MonitoringService 兼容委托；早期项目交付入口中的 Project generate/preview、Preset、Git、旧 Domain 和旧 CDN 配置生成也已接入团队上下文和控制面访问策略；但证书库/上传绑定、真实备份恢复、agent 级持续日志 follow runtime、真实 SLO 周期/多周期错误预算策略、e2e 权限覆盖、完整 agent supervisor 和 agent executor 仍待补。
