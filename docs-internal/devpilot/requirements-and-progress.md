...
## 8. F381 新手项目交付路径（2026-07-26）

- 项目详情已提供一条固定的六阶段路径：项目条目、仓库与解析事实、
  项目环境、资源与环境关联、应用与服务、计划/审批/执行。
- 入口页只给一个推荐主动作，并显示每一步的真实证据；项目 Tab 可通过
  URL 恢复，资源申请和应用创建可保留项目/环境上下文。
- 既有项目接入改为五步手工配置向导，明确说明当前不会连接、克隆或扫描
  仓库，仓库、分支、技术栈和命令均标记为未验证。
- 资源申请现在提交真实 `environmentId`；资源批量关联从仅 dry-run 却提示
  成功，改为预览、输入环境名确认、`dryRun:false` 应用和结果回读。
- 部署 dry-run 明确为只生成计划且不连接/执行；项目健康忽略 dry-run，
  部署记录可展开查看目标、环境、应用、服务、服务器、审批、执行任务、
  脱敏命令/变量、错误、日志和结果。
- Web type-check、生产构建、本地 Docker 页面更新和真实浏览器非破坏回归
  均通过。完整证据见
  `docs/devpilot/guided-delivery-2026-07-26/REPORT.md`。
- 后续仍需实现真实只读仓库连接/解析运行、统一活动时间线、provider 侧
  资源回收与凭据吊销，以及外部生产目标验收。

## 9. F382 部署前置阶段与业务初始化（2026-07-27）

- 状态：已完成代码、迁移、UI、Picshare 接入文档和自动化验证，本地
  3120/3121 实例已更新且健康；新浏览器会话无登录态，因此已登录页面的
  交互回归仍需在有登录态的浏览器补做。
- 平台需要让应用服务显式声明迁移、一次性业务初始化和启动前校验命令；
  不根据技术栈或仓库内容猜测是否执行 seed。
- 配置了前置阶段的正式部署必须按顺序执行、失败即阻断，并在部署记录中
  保存阶段、命令、触发策略、状态、时间、日志和错误证据。
- 未配置前置阶段的现有服务保持兼容，不改变既有构建/部署行为。
- Picshare 的迁移依赖管理员 seed、生产启动未调用 seed、迁移失败后仍启动
  服务属于项目内缺陷；Devpilot 的责任是提供显式能力并阻止假成功。
- 已实现 `preStartCheckCommand`、`migrationCommand`、
  `initializationCommand`，并固定在部署启动前执行。SSH/Server agent 都保留
  阶段策略和结果证据；失败阶段阻止后续启动。
- 一次性初始化使用“应用服务 + 项目环境 + 命令 SHA-256 指纹”检查点，
  成功后跳过、失败可重试、并发执行被阻断。含密钥的部署在安全重放能力
  完成前禁止排队。
