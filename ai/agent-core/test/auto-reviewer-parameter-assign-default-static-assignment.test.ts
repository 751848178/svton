...
describe('AutoReviewerManager parameter assignment default static assignments', () => {
  it('denies assignment defaults that are bypassed by known root variable values', async () => {
  ...
  it('keeps already-set safe or empty values from inheriting unsafe assignment fallbacks', async () => {
    const manager = new AutoReviewerManager({
      mode: 'auto_review',
      rules: BUILTIN_RULES,
    });
