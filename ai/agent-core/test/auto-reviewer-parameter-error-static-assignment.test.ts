...
describe('AutoReviewerManager parameter error static assignments', () => {
  it('denies error words that are bypassed by known root variable values', async () => {
  ...
  it('does not treat error words as destructive targets when Bash expands safe or empty values', async () => {
    const manager = new AutoReviewerManager({
      mode: 'auto_review',
      rules: BUILTIN_RULES,
    });
