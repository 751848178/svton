...
describe('AutoReviewerManager printf modified directive command substitutions', () => {
  it('denies executable tokens produced by static width and precision directives', async () => {
    ...
    await expect(manager.review(bashContext("$(printf '%.1s%.1s' root more) -rf /"))).resolves.toMatchObject({
      verdict: 'deny',
      ruleId: 'bash-rm-rf-root',
    });
    await expect(
