...
describe('AutoReviewerManager logical static assignment side effects', () => {
  it('respects && short-circuit assignment side effects before later commands', async () => {
  ...
  it('respects || short-circuit assignment side effects before later commands', async () => {
    ...
    await expect(
      manager.review(bashContext('false || target=/; find "$target" -delete')),
  ...
  it('keeps dynamic logical conditions conservative', async () => {
    const manager = new AutoReviewerManager({
      mode: 'auto_review',
      rules: BUILTIN_RULES,
    });
    ...
    await expect(
      manager.review(bashContext('"$CHECK" && target=/; rm -rf "$target"')),
    ).resolves.toMatchObject({ verdict: 'ask_user' });
    await expect(
      manager.review(bashContext('target=/; "$CHECK" && target=/tmp/project; rm -rf "$target"')),
    ).resolves.toMatchObject({ verdict: 'deny', ruleId: 'bash-rm-rf-root' });
  });
});
