...
describe('AutoReviewerManager shell parameter alternate words', () => {
  it('denies rm targets that alternate shell variables to root or home', async () => {
  ...
  it('denies remote pipelines that alternate shell variables to fetch or shell commands', async () => {
    const manager = new AutoReviewerManager({
      mode: 'auto_review',
      rules: BUILTIN_RULES,
    });
  ...
  it('leaves safe or literal alternate words user-reviewable', async () => {
    const manager = new AutoReviewerManager({
      mode: 'auto_review',
      rules: BUILTIN_RULES,
    });
