...
describe("ReleaseStagingService", () => {
  ...
  const service = new ReleaseStagingService(repository as never, executor as never);
  const context = {
    id: "order-1",
  ...
  it("creates a fresh DeploymentRun for each exact-Manifest request", async () => {
    ...
    expect(repository.finish).toHaveBeenLastCalledWith(expect.objectContaining({
      deploymentRunId: "deployment-2",
      status: "completed",
      result: expect.objectContaining({ buildInvoked: false, gitInvoked: false }),
    }));
  });

  it("rejects cross-order or unknown Manifests before creating a run", async () => {
    repository.manifest.mockResolvedValue(null);
    await expect(service.deploy(input())).rejects.toBeInstanceOf(NotFoundException);
  ...
  it("retains a failed DeploymentRun when exact artifact execution fails", async () => {
    executor.deploy.mockRejectedValue(new ReleaseStagingExecutionError({
      code: "ARTIFACT_DIGEST_MISMATCH",
      message: "digest mismatch",
      logs: ["failed"],
    }));
    await expect(service.deploy(input())).resolves.toMatchObject({ status: "failed" });
