...
describe("Deploy release gate providers", () => {
  ...
  it("reports positive environment-scoped M06-M09 evidence", () => {
    ...
    expect(
      registry
        .list(context)
        .filter((item) =>
          (["M06", "M07", "M08", "M09"] as ReleaseGateCapabilityId[]).includes(
            item.id,
          ),
        )
  ...
  it("blocks plaintext, cross-environment, and failed ownership evidence", () => {
    ...
    revision.secretReferences = [
      { id: "secret-1", name: "API", type: "api_key", value: "plaintext" },
    ];
  ...
  it("accepts an exact frozen target before its first DeploymentRun", () => {
    const context = evidenceContext();
    const environment = context.deploy!.environment!;
    context.deploy!.deployments = [];
    context.decisionTarget = {
      environmentId: environment.id,
      configRevisionId: environment.currentConfigRevision!.id,
      manifestId: "manifest-1",
    };
    expect(evaluate(registry, context).D01).toMatchObject({
      status: "checked",
      reasonCode: "deployment_target_frozen",
    });
  });

  it("binds post-execution D01 to the exact current DeploymentRun", () => {
    const context = evidenceContext();
    context.deploy!.deployments[0].status = "running";
    context.decisionTarget = {
      environmentId: context.deploy!.environment!.id,
      configRevisionId: context.deploy!.environment!.currentConfigRevision!.id,
      manifestId: "manifest-1",
      deploymentRunId: "deploy-1",
    };
    expect(evaluate(registry, context).D01.status).toBe("checked");
    context.decisionTarget.deploymentRunId = "foreign-run";
    expect(evaluate(registry, context).D01).toMatchObject({
      status: "blocked",
      reasonCode: "deployment_run_mismatch",
    });
  });
    ...
    deploy: {
      environment: {
        ...
        currentConfigRevision: {
          ...
          plainVariables: { API_URL: "https://api.test" },
