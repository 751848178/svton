...
describe("ReleaseProductionService preflight evidence boundary", () => {
  ...
  it("keeps GET preview evidence collection read-only", async () => {
    ...
    expect(preflight.preview.mock.calls[0][0]).not.toHaveProperty(
      "refreshEvidence",
    );
  });
  ...
  it("marks only the authorized refresh path as evidence-writing", async () => {
    ...
    expect(preflight.preview).toHaveBeenCalledWith(
  ...
  it("confirms the first release from exact preflight facts without a ReleaseRun fixture", async () => {
    ...
    const exactService = new ReleaseProductionService(
      exactRepository as never,
      capabilities as never,
      exactPreflight,
      concurrency as never,
    );
    await expect(
      exactService.confirm({
        teamId: "team-1",
        projectId: "project-1",
        releaseOrderId: "order-1",
        manifestId: "manifest-1",
        actorId: "actor-1",
        expectedInputHash: exactPreview.inputHash,
        idempotencyKey: "first-1",
      }),
    ).resolves.toEqual({ id: "release-1" });
    expect(gates.preview).toHaveBeenCalledWith(
      expect.objectContaining({
    ...
    expect(exactRepository.confirm).toHaveBeenCalledWith(
      expect.objectContaining({
...
function previewGateFromRealApplicability() {
  ...
  return {
    preview: jest.fn(async (input: any) => {
