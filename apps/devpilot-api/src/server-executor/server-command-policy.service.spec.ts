...
describe('ServerCommandPolicyService built-in rules', () => {
  it('allows the bounded docker stats JSON snapshot command', async () => {
    ...
    const service = new ServerCommandPolicyService(prisma as unknown as PrismaService);
  ...
  it('allows the bounded OpenSSL site TLS probe command only for the TLS probe operation', async () => {
    ...
    const service = new ServerCommandPolicyService(prisma as unknown as PrismaService);
    const command = 'echo | openssl s_client -servername api.example.com -connect api.example.com:443 2>/dev/null | openssl x509 -noout -subject -issuer -serial -dates -fingerprint -sha256';
    ...
    expect(wrongOperation.decisions[0]).toEqual(expect.objectContaining({
  ...
  it('allows bounded certbot renew commands only for the TLS renewal operation', async () => {
    ...
    expect(result.decisions[1]).toEqual(expect.objectContaining({
      status: 'allowed',
