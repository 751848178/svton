...
import { RepositoryAnalysisCompletionRepository } from "../repository-analysis/repository-analysis-completion.repository";
...
import { RepositoryAnalysisWorkerFinalizationService } from "../repository-analysis/repository-analysis-worker-finalization.service";
...
export function createIntakeService(client: PrismaClient, localRoot: string) {
  ...
  const config = new ConfigService({
    REPOSITORY_ANALYSIS_LOCAL_ROOTS: localRoot,
  });
  ...
  const completion = new RepositoryAnalysisCompletionRepository(db);
  const worker = new RepositoryAnalysisWorkerService(
    config,
    runRepository,
    new RepositoryAnalysisStageRepository(db),
    credentials,
    gitExecutor,
    new RepositoryInventoryService(config),
    new RepositoryParserService(),
    new RepositorySuggestionBuilderService(db),
    completion,
    new RepositoryAnalysisWorkerFinalizationService(completion),
  );
  ...
  const reviews = new RepositoryIntakeReviewService(
    contractRepository,
    contracts,
    apply,
  );
  ...
  return new ProjectIntakeService(
    db,
    new ProjectRepositoryDuplicateGuardService(db),
    connections,
    runs,
    contracts,
    reviews,
...
export async function createRepository(root: string) {
  ...
  await writePackage(root, "worker", {
    name: "worker",
    scripts: { build: "tsc", start: "node dist/worker.js" },
  });
  await writeFile(
    join(root, "apps/api/Dockerfile"),
    dockerfile("dist/main.js"),
  );
  await writeFile(
    join(root, "apps/worker/Dockerfile"),
    dockerfile("dist/worker.js"),
  );
  for (const args of [
    ["init", "-b", "main"],
    ["config", "user.email", "f415@example.com"],
    ["config", "user.name", "F415"],
    ["add", "."],
    ["commit", "-m", "fixture"],
  ])
...
export async function repositoryFingerprint(root: string) {
  const [{ stdout: commit }, { stdout: tree }, { stdout: status }] =
    await Promise.all([
