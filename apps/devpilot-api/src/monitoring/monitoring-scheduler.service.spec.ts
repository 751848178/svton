...
describe("MonitoringSchedulerService", () => {
  ...
  it("returns disabled summary when scheduler is off", async () => {
    ...
    await expect(
      service.runOnce(new Date("2026-06-27T12:00:00.000Z")),
    ).resolves.toEqual({
      skipped: false,
      enabled: false,
      scanned: 0,
  ...
  it("evaluates due scheduled rules as system actor", async () => {
    ...
    expect(monitoringService.evaluateRule).toHaveBeenNthCalledWith(
      1,
      "team-1",
      null,
      "rule-1",
  ...
  it("returns skipped summary when a scheduler tick is already running", async () => {
    prisma.alertRule.findMany.mockImplementation(
      () =>
        new Promise((resolve) => {
