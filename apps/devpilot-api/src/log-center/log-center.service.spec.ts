...
describe("LogCenterService stats", () => {
  ...
  let providerPlanService: LogProviderCollectionPlanService;
  ...
  beforeEach(() => {
    ...
    providerPlanService = new LogProviderCollectionPlanService(
  ...
  it("counts recent log entries by level for readable streams only", async () => {
    ...
    const stats = await service.getEntryStats("team-1", { windowMinutes: 30 }, [
      "stream-1",
      "stream-2",
    ]);
  ...
  it("returns empty stats without touching log entries when no streams are readable", async () => {
    const stats = await service.getEntryStats(
      "team-1",
      { windowMinutes: 60 },
      [],
    );
  ...
  it("uses the tail cursor to fetch only newer log entries", async () => {
    ...
    expect(result.cursor).toBe(
      "2026-06-27T00:00:03.000Z|2026-06-27T00:00:03.200Z|entry-4",
    );
  });

  it("creates a dry-run retention run without deleting entries", async () => {
    prisma.logStream.findFirst.mockResolvedValue(logStream());
    prisma.logRetentionRun.create.mockResolvedValue(
      logRetentionRun({ status: "running" }),
    );
    prisma.logEntry.count.mockResolvedValue(7);
    prisma.logRetentionRun.update.mockImplementation((args) =>
      Promise.resolve(
        logRetentionRun({
    ...
    const run = await service.cleanupRetention("team-1", "user-1", "stream-1", {
      dryRun: true,
    });
  ...
  it("deletes expired entries for live retention cleanup and refreshes stream metadata", async () => {
    prisma.logStream.findFirst.mockResolvedValue(logStream());
    prisma.logRetentionRun.create.mockResolvedValue(
      logRetentionRun({ dryRun: false, status: "running" }),
    );
  ...
  it("builds an Aliyun SLS GetLogs dry-run collection plan", async () => {
    const result = await providerPlanService.buildPlan(
      slsLogStream(),
      "collection-run-1",
      {
        dryRun: true,
    ...
    const plannedCalls = commandPlan.plannedCalls as Array<{
      operation: string;
    ...
    expect(runResult).toEqual(
      expect.objectContaining({
        ...
        executed: false,
        adapterKey: "aliyun-sls-query-plan",
        stdoutPreview: expect.stringContaining(
          'SLS dry-run query="level:error"',
        ),
      }),
    );
  });

  it("blocks unsupported provider collection plans without shell execution", async () => {
    const result = await providerPlanService.buildPlan(
      logStream({
        sourceType: "manual",
        managedResourceId: "resource-1",
        managedResource: { provider: "custom-provider" },
      }),
      "collection-run-unsupported",
      { dryRun: true, tail: 50 },
    );
    const commandPlan = result.commandPlan as Record<string, unknown>;
    ...
    expect(result).toEqual(
      expect.objectContaining({
        ...
        adapterKey: "manual-log-provider-plan",
        error: expect.stringContaining("暂不支持自动采集"),
      }),
    );
    expect(commandPlan).toEqual(
      expect.objectContaining({
  ...
  it("blocks live SLS collection until live SLS query gates are satisfied", async () => {
    ...
    const commandPlan = result.commandPlan as Record<string, unknown>;
    const livePrerequisites = commandPlan.livePrerequisites as Record<
      string,
      unknown
    >;
    const runResult = result.result as Record<string, unknown>;
    ...
    expect(result).toEqual(
      expect.objectContaining({
        ...
        error: expect.stringContaining(
          "LOG_CENTER_SLS_LIVE_QUERY_ENABLED=true",
        ),
      }),
    );
    expect(livePrerequisites).toEqual(
      expect.objectContaining({
  ...
  it("executes confirmed live SLS collection through the provider adapter", async () => {
    ...
    slsLogQueryAdapter.query.mockResolvedValue({
      ...
      logs: [
        {
          level: "info",
          message: "SLS GetLogs live 查询完成: prod-sls-project/app-log 1 条",
        },
      ],
      result: {
    ...
    expect(slsLogQueryAdapter.query).toHaveBeenCalledWith(
      expect.objectContaining({
        teamId: "team-1",
  ...
  it("redacts manually appended log entries with the stream policy", async () => {
    prisma.logStream.findFirst.mockResolvedValue(
      logStream({
        metadata: {
          redaction: {
    ...
    prisma.logEntry.create.mockImplementation((args) =>
      Promise.resolve({
        id: "entry-1",
    ...
    prisma.logStream.update.mockImplementation((args) =>
      Promise.resolve(logStream(args.data)),
    );

    await service.appendEntries("team-1", "user-1", "stream-1", {
      level: "info",
      message:
        "INFO session_id=abc user=ops@example.test ip=10.0.0.5 token=secret",
      raw: {
        session_id: "abc",
        nested: {
          ip: "10.0.0.5",
    ...
    expect(prisma.logEntry.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          message:
            "INFO session_id=[redacted] user=[redacted-email] ip=[redacted-ip] token=[redacted]",
