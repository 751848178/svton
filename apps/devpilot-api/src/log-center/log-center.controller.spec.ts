...
describe('LogCenterController tail', () => {
  it('checks log read policy before returning stream tail entries', async () => {
    ...
    expect(logCenterService.getStreamAccessScope).toHaveBeenCalledWith('team-1', 'stream-1');
    ...
    expect(logCenterService.tailStreamEntries).toHaveBeenCalledWith('team-1', 'stream-1', { limit: 25 });
  ...
  it('checks log read policy before opening stream tail events', async () => {
    ...
    expect(logCenterService.tailStreamEntries).toHaveBeenCalledWith('team-1', 'stream-1', {
      cursor: undefined,
      limit: 25,
    });
  ...
  it('emits session metadata and closes stream sessions at max duration', async () => {
    ...
    try {
      ...
      expect(response.write).toHaveBeenCalledWith(expect.stringContaining('"maxSessionMs":30000'));
      expect(response.write).toHaveBeenCalledWith(expect.stringContaining('"sessionId":'));
      expect(response.write).toHaveBeenCalledWith(expect.stringContaining('"maxActorActiveSessions"'));
      expect(response.write).toHaveBeenCalledWith(expect.stringContaining('"maxTeamActiveSessions"'));
      jest.advanceTimersByTime(30000);
      expect(response.write).toHaveBeenCalledWith('event: closing\n');
      expect(response.write).toHaveBeenCalledWith(expect.stringContaining('"reason":"max_session_duration"'));
  ...
  it('lists and closes active stream sessions through policy checks', async () => {
    ...
    expect(response.write).toHaveBeenCalledWith('event: closing\n');
    expect(response.write).toHaveBeenCalledWith(expect.stringContaining('"reason":"manual_close"'));
    expect(response.end).toHaveBeenCalled();
    await expect(controller.listStreamSessions(
      { teamId: 'team-1', user: { id: 'user-1' } },
      'stream-1',
    )).resolves.toEqual([]);
  });
  ...
  it('limits active stream sessions per log stream', async () => {
    ...
    try {
      ...
      await expect(controller.streamTailEvents(
        {
          teamId: 'team-1',
          user: { id: 'user-1' },
          on: jest.fn(),
