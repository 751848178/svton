...
describe("LogCollectionIngestionService", () => {
  ...
  let repository: LogCollectionIngestionRepository;
  ...
  beforeEach(() => {
    ...
    repository = new LogCollectionIngestionRepository(
      prisma as unknown as PrismaService,
    );
    service = new LogCollectionIngestionService(repository);
  });
    ...
    expect(prisma.logEntry.createMany).toHaveBeenCalledWith({
      data: [
        ...
        expect.objectContaining({
          level: "error",
          message:
            "2026-06-27T00:00:01Z ERROR password=[redacted] token=[redacted] failed",
  ...
  it("applies stream redaction metadata to collected log output", async () => {
    prisma.logCollectionRun.findFirst.mockResolvedValue({
      ...
      result: {
        stdoutPreview:
          "2026-06-27T00:00:00Z INFO session_id=abc user=ops@example.test ip=10.0.0.5",
      },
      logs: [],
    ...
    expect(prisma.logEntry.createMany).toHaveBeenCalledWith({
      data: [
        expect.objectContaining({
          message:
            "2026-06-27T00:00:00Z INFO session_id=[redacted] user=[redacted-email] ip=[redacted-ip]",
