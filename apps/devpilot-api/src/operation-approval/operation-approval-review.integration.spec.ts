...
import { OPERATION_APPROVAL_REVIEW_CONFLICT, OperationApprovalReviewService } from "./operation-approval-review.service";
...
function twoPartyBarrier() {
  ...
  return {
    ...
    letGo() { a.gate.resolve(); b.gate.resolve(); },
  };
}

describeIntegration("F470 OperationApproval concurrent review (real MySQL CAS)", () => {
  ...
  function buildReviewService(opts: {
    ...
    if (opts.auditOverride) {
      (auditService as any).writeApprovalAudit = opts.auditOverride.writeApprovalAudit;
    }
    return new OperationApprovalReviewService(
      prismaService, repo, auditService, opts.accessPolicy as any,
    );
  }
  ...
  async function seed() {
    const suffix = rnd();
    const team = await prisma.team.create({ data: { name: `team-${suffix}` } });
    const requester = await prisma.user.create({ data: { email: `req-${suffix}@x.test` } });
    const reviewerA = await prisma.user.create({ data: { email: `rva-${suffix}@x.test` } });
    const reviewerB = await prisma.user.create({ data: { email: `rvb-${suffix}@x.test` } });
    const approval = await prisma.operationApproval.create({
      data: {
        teamId: team.id, requesterId: requester.id, category: "resource_action",
        action: "resource.action.restart", targetType: "managed_resource",
        risk: "high", status: "pending", summary: `seed-${suffix}`,
      },
    });
  ...
  async function cleanup(suffix: string) {
    await prisma.auditEvent.deleteMany({ where: { summary: { contains: suffix } } }).catch(() => {});
    await prisma.operationApproval.deleteMany({ where: { summary: { contains: suffix } } }).catch(() => {});
    // 种子 emails 形如 `req-${suffix}@x.test`，按后缀串精确匹配本批随机用户。
    await prisma.user.deleteMany({ where: { email: { contains: `-${suffix}@` } } }).catch(() => {});
    await prisma.team.deleteMany({ where: { name: { contains: suffix } } }).catch(() => {});
  }
  ...
  it("two reviewers pass barrier into real CAS → exactly one wins, one 409; DB row + sole audit belong to winner", async () => {
    ...
    try {
      ...
      expect(loserErr.getResponse()).toMatchObject({ code: OPERATION_APPROVAL_REVIEW_CONFLICT });

      // DB 终态行属于胜者：status/reviewer/comment 与 fulfilled 结果一致，绝非输家。
      const winnerValue = (winner as PromiseFulfilledResult<any>).value;
      const dbRow = await prisma.operationApproval.findUniqueOrThrow({ where: { id: approval.id } });
      ...
      const audits = await prisma.auditEvent.findMany({
        where: { operationApprovalId: approval.id, action: { in: ["approval.approved", "approval.rejected"] } },
      });
      expect(audits).toHaveLength(1);
      expect(audits[0].actorId).toBe(winnerValue.reviewerId);
      expect(audits[0].action).toBe(
        winnerValue.status === "approved" ? "approval.approved" : "approval.rejected",
      );
  ...
  it("replaying the opposite terminal review returns 409, leaves row unchanged, still one decision audit", async () => {
    ...
    try {
      ...
      const first = await service.review(team.id, reviewerA.id, approval.id, { decision: "approved" });
      expect(first?.status).toBe("approved");

      // 相反决策重放：CAS 谓词 status:pending 命中 0 行 → 409。
      await expect(
        service.review(team.id, reviewerB.id, approval.id, { decision: "rejected", reviewComment: "late" }),
  ...
  it("audit throwing inside the transaction rolls back the decision to pending with no audit", async () => {
    ...
    try {
      const service = buildReviewService({
        ...
        auditOverride: {
          writeApprovalAudit: async () => { throw new Error("audit-down"); },
        },
      });

      await expect(
        service.review(team.id, reviewerA.id, approval.id, { decision: "approved" }),
  ...
  it("approved row consumes once, second consume affects zero, re-review stays 409 with stable fields", async () => {
    ...
    try {
      const service = buildReviewService({ accessPolicy: allowAll });
      const approved = await service.review(team.id, reviewerA.id, approval.id, { decision: "approved" });
      ...
      await expect(
        service.review(team.id, reviewerA.id, approval.id, { decision: "rejected" }),
      ).rejects.toThrow(ConflictException);
