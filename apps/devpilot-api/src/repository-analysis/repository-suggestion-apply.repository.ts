...
import { orderRepositoryDecisions } from "./repository-suggestion-order.utils";
...
export class RepositorySuggestionApplyRepository {
  ...
  apply(input: RepositoryApplyInput) {
    return this.prisma.$transaction(async (tx) => {
      ...
      const ordered = orderRepositoryDecisions(input.decisions);
  ...
  private async applyDecision(
    ...
    if (suggestion.runId !== input.runId)
  ...
  private applyValue(
    ...
    switch (suggestion.kind) {
      case "environment":
        return this.platform.applyEnvironment(
          tx,
          input.teamId,
          input.projectId,
          value,
      ...
      case "project_repository":
        return this.platform.applyProject(
          tx,
          input.projectId,
          input.runId,
          value,
      ...
      case "resource_requirement":
        return this.platform.applyResourceRequirements(
          tx,
          input.projectId,
          value,
        );
