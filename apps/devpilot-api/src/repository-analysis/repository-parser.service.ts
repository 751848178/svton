...
export class RepositoryParserService {
  parse(inventory: RepositoryInventory): RepositoryAnalysisResult {
    ...
    const resourceRequirements = [...new Set(
      services.flatMap((service) => service.databases),
    )].sort();
    return {
      repository: {
        ...repository,
  ...
  private collectWarnings(
    ...
    if (composeCount > 1) {
      warnings.push(`检测到 ${composeCount} 份 Compose 配置，应用前必须确认部署目标。`);
    }
    for (const service of services) {
      const paths = [...new Set(service.healthChecks.map((item) => item.path))];
      if (paths.length > 1) {
        warnings.push(`${service.name} 存在多个健康检查 ${paths.join('、')}，需要确认发布门禁。`);
      }
      if (!service.deployable && !service.artifactOnly && service.role !== 'shared') {
