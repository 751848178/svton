...
describe('RepositoryParserService', () => {
  ...
  it('detects the Picshare delivery shape without treating the workspace root as a service', () => {
    ...
    const byPath = new Map(result.services.map((service) => [service.path, service]));
    ...
    expect(result.services.some((service) => service.name === 'picshare')).toBe(false);
    ...
    expect(byPath.get('apps/backend')?.healthChecks.map((item) => item.path)).toEqual(
      expect.arrayContaining(['/api/health/readiness', '/api']),
    );
...
function picshareInventory(): RepositoryInventory {
  const manifests: Record<string, string> = {
    ...
    'apps/backend/Dockerfile': 'COPY package.json pnpm-lock.yaml ./\nEXPOSE 3000\n',
    'apps/admin/Dockerfile': 'COPY package.json pnpm-lock.yaml ./\nEXPOSE 3001\n',
    'apps/backend/prisma/schema.prisma': 'datasource db { provider = "mysql" }\n',
