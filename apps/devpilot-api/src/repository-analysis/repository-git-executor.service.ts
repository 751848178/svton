...
import {
  ...
  createRepositoryGitWorkspace,
} from "./repository-git-workspace.utils";
...
export class RepositoryGitExecutorService {
  ...
  constructor(
    ...
    this.localRoots = (
      config.get<string>("REPOSITORY_ANALYSIS_LOCAL_ROOTS") || ""
    )
      .split(delimiter)
      .map((item) => item.trim())
      .filter(Boolean)
  ...
  async resolveRef(
    ...
    const defaultBranch = headOutput.stdout.match(
    ...
    const selectedBranch = validateRepositoryBranch(
    ...
    const commitSha = headsOutput.stdout
      .match(branchPattern)?.[1]
      ?.toLowerCase();
    if (!commitSha) {
      throw new RepositoryGitError({
  ...
  async checkout(
    ...
    const { root, cleanup } = await createRepositoryGitWorkspace(workspace);
    try {
      ...
      await this.runGit(
        [
          ...
          "--depth=1",
          "origin",
          `refs/heads/${validateRepositoryBranch(branch)}`,
        ],
        credential,
        root,
        signal,
  ...
  private localPath(repositoryUrl: string): string | null {
    ...
    if (repositoryUrl.startsWith("file://"))
      return resolve(new URL(repositoryUrl).pathname);
    return null;
  }
}
