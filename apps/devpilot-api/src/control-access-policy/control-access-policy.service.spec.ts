...
describe("ControlAccessPolicyService", () => {
  ...
  beforeEach(() => {
    ...
    const policyRepository = new ControlAccessPolicyRepository(
      prisma as unknown as PrismaService,
    );
    const policyAuditService = new ControlAccessPolicyAuditService(
      auditEventService as unknown as AuditEventService,
    );
    const policyAccessService = new ControlAccessPolicyAccessService(
      policyRepository,
    );
    const policyCrudService = new ControlAccessPolicyCrudService(
      policyRepository,
      policyAuditService,
    );
  ...
  it("gives deny policies priority over matching allow policies", async () => {
    ...
    await expect(
      ...
      "控制面访问策略「拒绝读取生产部署」拒绝 deployment/deployment_run.read",
    );
  ...
  it("keeps sensitive reads admin-only without an explicit allow policy", async () => {
    ...
    await expect(
      service.assertCanSensitiveRead({
        teamId,
        actorId,
        projectId,
        environmentId,
