...
/** 脱敏展示投递目标：仅显示前 20 字符，超出加省略号；完整值置于 title。 */
function maskTarget(target: string): { text: string; title: string } {
  const text = target.length > 20 ? `${target.slice(0, 20)}…` : target;
  return { text, title: target };
}
      ...
      {m.notificationDeliveries.length === 0 ? (
        ...
        <div className="divide-y">
          {m.notificationDeliveries.map((delivery) => {
            const target = delivery.target ? maskTarget(delivery.target) : null;
            return (
              <div
                key={delivery.id}
                className="px-4 py-3"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    {delivery.error ? (
                      <div className="mt-1 break-words text-xs text-destructive">{delivery.error}</div>
                    ) : null}
                  </div>
