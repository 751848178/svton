 ...
 * 影响之后创建的运行）。当前版本只读展示生效策略，不提供修改入口。
...
import { ReleasePolicyGates } from './release-policy-gates';
...
export function ReleasePolicyTab({ projectId }: { projectId: string }) {
  ...
  return (
    <div className="max-w-4xl space-y-4">
      <Card title={t('releasePolicyTitle')}>
        ...
        <dl className="mb-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">
          <Value
            label={t('releasePolicyFactReleaseOrder')}
            value={t('releasePolicyFactReleaseOrderValue')}
          />
          <Value
            label={t('releasePolicyFactArtifactPolicy')}
            value={t('releasePolicyFactArtifactPolicyValue')}
          />
          <Value
            label={t('releasePolicyFactProductionProtection')}
            value={t('releasePolicyApprovalRequired')}
          />
          <Value
            label={t('releasePolicyFactConcurrency')}
            value={t('releasePolicyFactConcurrencyValue')}
          />
        </dl>
        ...
        <div className="mb-4 flex flex-wrap items-center gap-3 rounded-lg border bg-muted/40 px-3 py-2.5">
          ...
          <span className="text-xs text-muted-foreground">
            {t('releasePolicyStandardCardDescription')}
          </span>
        </div>
          ...
          {policy.policy.capabilities.map((capability) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                >
                  {capability.executable
                    ? t('releasePolicyAvailable')
                    : t('releasePolicyUnavailable')}
                </span>
              </div>
      ...
      <Card title={t('releasePolicyTargetRulesTitle')}>
        <dl className="grid gap-3 text-sm sm:grid-cols-2">
          <Value
            label={t('releasePolicyTargetRulesChangeWindow')}
            value={t('releasePolicyTargetRulesUnavailable')}
          />
          <Value
            label={t('releasePolicyTargetRulesFreeze')}
            value={t('releasePolicyTargetRulesUnavailable')}
          />
        </dl>
        <p className="mt-3 text-xs text-muted-foreground">
          {t('releasePolicyTargetRulesEnforced')}
        </p>
      </Card>
