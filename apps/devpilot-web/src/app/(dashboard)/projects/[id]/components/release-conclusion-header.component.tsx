...
export function ReleaseConclusionHeader(props: ReleaseConclusionHeaderProps): JSX.Element {
  const { plan, conclusion, capability, preview, loadingExecute, loadingCancel, onExecute, onCancel } = props;
  ...
  const canCancel = capability?.canCancel !== false &&
    !['succeeded', 'failed', 'canceled'].includes(plan.status);
  const envName = plan.environment?.name ?? plan.environmentId.slice(-6);
  const executeTooltip = !flagOn
    ? '发布编排未启用'
    : !canWrite
      ? '无写权限'
      : !canExecute
    ...
    <Card>
      <div className="space-y-2">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {plan.branch && <span className="text-xs text-muted-foreground">分支：{plan.branch}</span>}
          {plan.commitSha && (
            <span className="text-xs text-muted-foreground">提交：{plan.commitSha.slice(0, 8)}</span>
          )}
        ...
        <div className="flex gap-2 pt-1">
          {canExecute ? (
            <Button onClick={() => setShowExecute(true)} loading={loadingExecute} title={executeTooltip}>
              开始执行
            </Button>
          ) : (
            executeTooltip && (
              <Button disabled title={executeTooltip}>
                开始执行
              </Button>
            )
          )}
          {canCancel && (
            <Button variant="outline" onClick={() => setShowCancel(true)} loading={loadingCancel}>
              取消发布
            </Button>
          )}
        </div>
