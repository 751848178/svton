import { serverRequest } from '@/lib/api-client/server';

import type { ResourceInstance } from './types';
import { ResourceInstancesContent } from './components/ResourceInstancesContent';

/** 该页在请求时读取 cookies() 鉴权，必须动态渲染。 */
export const dynamic = 'force-dynamic';

/**
 * 资源实例 — Server Component。
 *
 * 首屏在服务端取数（走 cookie 鉴权），下发 initialInstances 给客户端 ResourceInstancesContent（SWR fallback）。
 * 释放动作由 ResourceInstancesContent 承担。
 *
 * 注意：server 取到空列表时不传 fallback（传 undefined），让客户端 SWR 走正常 loading 流程，
 * 避免空数组被当作「已有数据」而不发起请求。
 */
export default async function ResourceInstancesPage() {
  let initialInstances: ResourceInstance[] | undefined;
  try {
    const data = await serverRequest<ResourceInstance[]>('GET:/resource-instances');
    initialInstances = data.length > 0 ? data : undefined;
  } catch (error) {
    console.error('Failed to load resource instances:', error);
  }

  return <ResourceInstancesContent initialInstances={initialInstances} />;
}
