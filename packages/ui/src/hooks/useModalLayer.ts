...
interface InertSnapshot { attribute: string | null; property: boolean }
...
export function useModalLayer(
  ...
  useLayoutEffect(() => {
    ...
    const layer: Layer = {
      id: idRef.current,
      opener: optionsRef.current.openerRef?.current
...
function restoreLayerFocus(layer: Layer, attempts = 20): void {
  ...
  const fallback = restoreFocusSelector
    ? document.querySelector<HTMLElement>(restoreFocusSelector) : null;
...
function isSafeFocusTarget(target: HTMLElement | null | undefined): target is HTMLElement {
  if (!target?.isConnected || target.closest('[inert],[hidden],[aria-hidden="true"]')) return false;
